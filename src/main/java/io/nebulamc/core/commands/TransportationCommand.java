    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        ...
        if(cmd.getLabel().equalsIgnoreCase("boat")) {
            if(!player.getLocation().add(0.0, -1.0, 0.0).getBlock().getType().toString().endsWith("ICE")) {
                sender.sendMessage(ChatColor.RED + "You must be on an ice block to run this command.");
                return true;
            }
            ...
            if(!player.getLocation().getBlock().getType().toString().endsWith("RAIL")) {
                sender.sendMessage(ChatColor.RED + "You must be on railroad to run this command.");
                return true;
            }
